...
class VideoRecord(object):
    ...
    def start_frame(self):
        return int(self.start_sec * self.fps)

    @property
    def end_frame(self):
        return int(self.end_sec * self.fps)
...
class VideoRecord(object):
    ...
    def __init__(self, row, root_datapath):
        ...
        self._data = row
        self._path = os.path.join(root_datapath, row[0])
        self.vidname = self._data[0]
        self.path = self._path
        self.start_sec = float(self._data[1])
        self.end_sec = float(self._data[2])
        self.fps = float(self._data[3])
        self.duration = float(self._data[4])
        self.caption = self._data[5]
        ...
        self.max_frame = int(self.duration * self.fps)
'''


class HowtoDataset(torch.utils.data.Dataset):
    r"""
    ...
    def __init__(self,
        ...
        super(HowtoDataset, self).__init__()
        """
    ...
    def _parse_list(self):
        ...
        for x in open(self.ann_file):
            vid, stim, etim, fps, duration = x.strip().split('#')[0].split()
            #print(f"{vid}/{stim}/{etim}/{fps}/{duration}")
            caption = x.strip().split('#')[1]
            video_list.append(VideoRecord([vid, stim, etim, fps, duration, caption], self.root))
        self.video_list = video_list
        #for vidrecord in self.video_list:
        #    print(vidrecord.fps)
    ...
    def __getitem__(self, index):
        ...
        shiftRecord = VideoRecord([record.vidname, sstart/record.fps, send/record.fps, record.fps, record.duration, record.caption], self.root)
        ...
        #tarInfo = tarfile.open(record.path + '.mp4')
        ...
        return self._get(record, segment_indices, shiftRecord, segment_indices_shift, selectIdx)

    def _get(self, record, indices, srecord, sindices, label):
        """
        ...
        for seg_ind in indices:
            ...
            segClip, _, _ = read_video("{}.mp4".format(record.path), frame_index/record.fps, (frame_index+self.frames_per_segment-1)/record.fps, pts_unit='sec')
            images.extend(segClip)
            # [FRAME, HEIGHT, WEIGHT, CHANNEL],[0, 255] -> [FRAME, CHANNEL, HEIGHT, WEIGHT],[0,1]
            #newSegClip = torch.stack([transforms.functional.to_tensor(pic) for pic in segClip.numpy()])
            # Note: DEBUG TO HERE
            '''
            for i in range(self.frames_per_segment):
                import ipdb; ipdb.set_trace()
                #ccclip, _, _ = read_video("{}.mp4".format(record.path), frame_index, frame_index+self.frames_per_segment, pts_unit='pts')
                segClip, _, _ = read_video("{}.mp4".format(record.path), frame_index/record.fps, (frame_index+self.frames_per_segment-1)/record.fps, pts_unit='sec')
        ...
        if images.shape[0] > 80:
            images = images[:80, :, :, :]
        if images.shape[0] < 80:
            FRAMENUM = images.shape[0]
            lastImg = images[-1, :, :, :].unsqueeze(0).expand(80-FRAMENUM, -1, -1, -1)
            images = torch.cat((images, lastImg), dim=0)
        ...
        for seg_ind in sindices:
            ...
            segClip, _, _ = read_video("{}.mp4".format(record.path), frame_index/record.fps, (frame_index+self.frames_per_segment-1)/record.fps, pts_unit='sec')
            simages.extend(segClip)
        simages = torch.stack(simages)
        if simages.shape[0] > 80:
            simages = simages[:80, :, :, :]
        if simages.shape[0] < 80:
            FRAMENUM = simages.shape[0]
            lastImg = simages[-1, :, :, :].unsqueeze(0).expand(80-FRAMENUM, -1, -1, -1)
            simages = torch.cat((simages, lastImg), dim=0)
...
class BatchTensorNorm(torch.nn.Module):
    def forward(self, segClip):
        """segClip: [FRAME, H, W, C] ([0, 255])"""
        #segClipNP = segClip.numpy()
        return torch.stack([transforms.functional.to_tensor(pic) for pic in segClip.numpy()])
